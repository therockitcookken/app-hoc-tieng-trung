...
export const GrammarCategorySubPageSlideOver: React.FC<GrammarCategorySubPageSlideOverProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[95] bg-slate-950/80 backdrop-blur-xl flex justify-end animate-fade-in select-none">
      ...
      <div className="w-full max-w-4xl h-full bg-[#0F172A] text-slate-100 flex flex-col shadow-2xl border-l border-slate-700/60 overflow-hidden transform transition-all duration-400 ease-[cubic-bezier(0.32,0.72,0,1)]">
        {/* Top Header Navigation Bar */}
        <div className="p-4 sm:p-6 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between sticky top-0 z-20 backdrop-blur-md">
          <div className="flex items-center space-x-3">
            ...
            <span className="eyebrow-pill bg-blue-500/20 text-blue-400 border-blue-500/30 hidden sm:inline-flex">
              {categoryMeta.badge}
            </span>
          </div>
          ...
          <button
            onClick={onClose}
            type="button"
            className="w-9 h-9 rounded-full bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center cursor-pointer transition-spring active:scale-90"
            aria-label="Đóng"
          >
            <X className="w-5 h-5" />
          </button>
